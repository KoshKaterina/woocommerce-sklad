...
- 🟢 Разнесение адреса доставки (2026-06, WC): кроме плоского `shipmentAddress` заполняем нативный объект `shipmentAddressFull` (индекс/страна/город/улица/дом/квартира). Источник — стандартизованный плагином DaData `shipping.address_1` (курьер) / `address_2` + мета CDEK (ПВЗ), парсится локально (`core/address_parser.py`). Код ПВЗ — из меты `_official_cdek_office_code` (первично). Страна резолвится в справочник МС по ISO-коду (`ms_client.find_country_meta`, не хардкод — бывают KZ/BY); регион не пишем, «Другое» (addInfo) не заполняем. Самовывоз из офиса — без адреса. Прод-заказы 8–11.06 вычищены ретро-скриптом (`scripts/fix_address_retro.py`). InSales (2026-06): тоже пишет `shipmentAddressFull` — ПВЗ из структурированной строки СДЭК `outlet.address`, курьер из КЛАДР-данных + DaData-строки покупателя; ретро существующих заказов — `scripts/fix_insales_addresses_retro.py` (нужен российский IP)

- 🟢 Поле «Доставка (СД)» согласовано с услугами (2026-06): интеграция авто-ставит только «СДЭК» (любой тариф СДЭК, WC и InSales), «Достависта (стандартная)» (WC, курьерка по Москве), «Самовывоз из шоурума Sunscrypt» (WC `local_pickup`) и «ExpressRMS(Самовывоз)» (InSales «Самовывоз из Шоурума», услуга «Самовывоз со склада»); работает и из вебхука, и из сверки (общий `OrderProcessor`). Остальные значения справочника (ускоренный/экспресс, Яндекс Доставка, Почта России) менеджер выбирает вручную. Дубль услуги «Самовывоз из офиса Sunscrypt» слит разовым скриптом `scripts/merge_pickup_services.py` (756 позиций перенесены, дубль удалён)
