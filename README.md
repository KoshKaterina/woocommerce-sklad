...
woo_moysklad/                  — пакет приложения (только он копируется в Docker-образ)
  ...
  core/                    — платформо-независимый конвейер заказа
    ...
    field_mappers.py       — маппинг полей → МС (адрес, тип доставки, ПВЗ, промокод, доп.поля)
...
tests/
  test_field_mappers.py, test_counterparty.py, test_order_processor.py,
  test_insales_normalizer.py, test_insales_e2e.py, test_reconciliation.py,
  test_ucoz.py, test_ucoz_normalizer.py
...
- ⚠️ Доп.поля МС обновлены (2026-06): «Вид доставки» теперь `long`, стоимости — `double`, «Прием платежа» — новый справочник. Прод-`.env` должен содержать новые UUID (см. `config.py`)
