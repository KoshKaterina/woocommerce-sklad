...
- [x] Для самовывоза из офиса атрибут "Доставка (СД)" не заполняем — только услуга "Самовывоз из офиса Sunscrypt" в позициях заказа. `MS_DELIVERY_SD_PICKUP_ID` удалён из config/.env. **Устарело (2026-06):** теперь самовывоз ставит значение «Самовывоз из шоурума Sunscrypt» (`MS_DELIVERY_SD_SHOWROOM_ID`, захардкожен); курьерка по Москве → «Достависта (стандартная)» вместо Яндекса. См. `map_delivery_sd` и CLAUDE.md.
...
- [x] `insales_normalizer.py` — маппинг полей InSales→NormalizedOrder:
  ...
  - Самовывоз из Шоурума → ~~`delivery_sd=None`, имя услуги = "Самовывоз из офиса Sunscrypt"~~ **с 2026-06:** `delivery_sd="rms_pickup"` («ExpressRMS(Самовывоз)»), услуга = "Самовывоз со склада" (см. §6)
...
## 6. Поле «Доставка (СД)» ↔ услуги (2026-06-11)
- [x] WC: СДЭК → «СДЭК», курьерка по Москве/Достависта → «Достависта (стандартная)», самовывоз из офиса (`local_pickup`) → «Самовывоз из шоурума Sunscrypt»; остальные значения — только вручную
- [x] InSales: СДЭК → «СДЭК»; «Самовывоз из Шоурума» → «ExpressRMS(Самовывоз)» + услуга «Самовывоз со склада» (вместо «Самовывоз из офиса Sunscrypt»)
- [x] Дубль услуги «Самовывоз из офиса Sunscrypt» слит (`scripts/merge_pickup_services.py`, 11.06): 756 позиций заказов/отгрузок переведены с 856617ad… на 850f2516…, дубль удалён. Проверено: в МС осталась одна услуга
- [ ] Услуга «Самовывоз со склада» создастся автоматически при первом InSales-самовывозе; при желании завести заранее руками (точное имя)
