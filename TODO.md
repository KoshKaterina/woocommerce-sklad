...
- [ ] **Сайт (не интеграция):** чекаут «Доставка курьером по Москве» (flat_rate) не передаёт адрес вообще (17083, 17109) — регрессия после DaData: до подключения поле заполнялось руками в 40/45 заказов, после — 0/2. Привязать DaData-поле к методу в чекауте
