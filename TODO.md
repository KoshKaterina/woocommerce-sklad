...
## 4. Обратная синхронизация полей из МС (реализовано, off по флагу)

При ручном изменении менеджером заказа в МС — пересчитываем зависимые доп.поля по тем же правилам, что и при создании. Модуль `core/field_resync.py`, включается `FIELD_RESYNC_ENABLED=true`.
