...
**Текущая реализация (на момент v2):** одно текстовое поле `shipmentAddress`.  
Разбивка на `shipmentAddressFull.*` — была отложена.

> ⚠️ Устарело (2026-06): разбивка **реализована** для WC — заполняется нативный
> `shipmentAddressFull` (индекс/страна/город/улица/дом/квартира). См. `core/address_parser.py`,
> `README.md` и `TODO.md §5`. Плоский `shipmentAddress` пишется по-прежнему (параллельно).
