...
При `order.updated` + `status == "processing"`:
1. Найти заказ(ы) в МС по доп. полю "Номер заказа на сайте" = `order_id` (и `order_id_1` для смешанных)
2. Для каждого найденного заказа создать входящий платёж:
   ```
   POST /entity/paymentin
   {
     "organization": meta организации,
     "agent": meta контрагента из заказа,
     "sum": сумма заказа (из поля sum),
     "operations": [{"meta": meta заказа}]
   }
   ```
3. Никакие другие поля заказа не трогаются
...
1. Получить заказы WC за последние 40 минут: `GET /orders?after={iso}&before={iso}&per_page=100`, пагинация
2. Для каждого: проверка наличия в МС по `attributes.{MS_ATTR_ORDER_NUMBER_ID}={order_id}`
3. Не найден → `order_processor.process_order()` с топиком `order.created` (создаёт новый)
4. Найден → пропустить (**никогда не обновлять**)
5. Лог: `"Сверка: проверено={N}, в МС={M}, создано={K}, ошибок={E}"`
...
6. Определить действие:
   - order.updated + status == "processing" → mark_paid (см. 6а)
   - order.created → создать заказ (см. 6б)

6а. mark_paid:
   - Найти заказ(ы) в МС по "Номер заказа на сайте" (основной + суффикс _1)
   - Для каждого: POST /entity/paymentin (organization, agent, sum, operations)
